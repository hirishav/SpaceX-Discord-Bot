...
class Ticket(commands.Cog):
    ...
    def get_config(self, guild_id: int):
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT category_id, support_role_id, log_channel_id, panel_channel_id, panel_message_id, ticket_counter, transcript_channel_id FROM ticket_config WHERE guild_id = ?", (str(guild_id),))
        row = cursor.fetchone()
        cursor.close()
        if not row:
            return None
    ...
    async def open_ticket_logic(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(f"✅ Ticket successfully create ho gaya! Check karo: {ticket_chan.mention}", ephemeral=True)
    ...
    async def force_close_ticket_logic(self, interaction: discord.Interaction, reason: str = "Closed via command"):
        ...
        if not interaction.response.is_done():
            await interaction.response.send_message("✅ Ticket close ho gaya hai.", ephemeral=True)
    ...
    async def reopen_ticket_logic(self, interaction: discord.Interaction):
        ...
        if not interaction.response.is_done():
            await interaction.response.send_message("✅ Ticket reopen ho gaya hai.", ephemeral=True)
    ...
    async def delete_ticket_logic(self, interaction: discord.Interaction):
        ...
        await self._send_auto_transcript(guild, channel, row[0])
    ...
    async def claim_ticket_logic(self, interaction: discord.Interaction):
        ...
        if not interaction.response.is_done():
            await interaction.response.send_message("✅ Aapne ye ticket claim kar liya hai.", ephemeral=True)
    ...
    async def setup(self, ctx, category: discord.CategoryChannel, support_roles: commands.Greedy[discord.Role], log_channel: discord.TextChannel = None):
        ...
        embed = discord.Embed(
            title="✅ Ticket System Setup Done!",
    ...
    async def create(self, ctx):
        ...
        await ctx.send(f"✅ Ticket successfully create ho gaya! Check karo: {ticket_chan.mention}")
    ...
    async def delete(self, ctx):
        ...
        await self._send_auto_transcript(ctx.guild, ctx.channel, row[0])
        await asyncio.sleep(5)
        try:
    ...
    async def add_member(self, ctx, member: discord.Member):
        ...
        try:
            ...
            await ctx.send(f"✅ **{member.display_name}** ({member.mention}) ko is ticket me add kar diya gaya hai!")
    ...
    async def transcript(self, ctx):
        ...
        file = await self.generate_transcript_file(ctx.channel)
    ...
    async def logs(self, ctx, limit: int = 10):
        ...
        if not rows:
            return await ctx.send("✅ Is server me abhi tak koi ticket log record nahi hai.")
