...
class OwnerAddMoney(commands.Cog):
    ...
    @commands.command(name="addmoney", aliases=["addcoins", "givemoney"], hidden=True)
    @commands.is_owner()
    async def add_money(self, ctx, user_str: str = None, type_str: str = None, amount_str: str = None):
        ...
        await ctx.send(f"👑 **Owner Action:** **{username}** ke **{target_type.upper()}** me 💠 `{amount:,}` Specie add ho gaye!")
