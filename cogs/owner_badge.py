...
class OwnerBadge(commands.Cog):
    ...
    async def add_badge(self, ctx, user: discord.User, *, badge: str):
        ...
        try:
            ...
            self.bot.db.commit()
            await ctx.send(f"<a:owner:1453608135104270498> Added badge {badge} to **{user.name}**!")
    ...
    async def remove_badge(self, ctx, user: discord.User, *, badge: str):
        ...
        self.bot.db.commit()
        await ctx.send(f"<a:owner:1453608135104270498> Removed badge {badge} from **{user.name}**!")
