...
class OwnerServers(commands.Cog):
    ...
    async def add_premium(self, ctx, server_id: int):
        ...
        try:
            ...
            await ctx.send(f"<:verified_tick:837551087786393710> Server `{server_id}` ko successfully **Premium** access de diya gaya hai!")
    ...
    async def remove_premium(self, ctx, server_id: int):
        ...
        try:
            ...
            await ctx.send(f"<:verified_tick:837551087786393710> Server `{server_id}` ka **Premium** access remove kar diya gaya hai!")
        except Exception as e:
