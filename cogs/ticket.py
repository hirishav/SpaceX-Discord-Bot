...
class Ticket(commands.Cog):
    ...
    async def open_ticket_logic(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(f"<a:verified_tick:837551087786393710> Ticket successfully create ho gaya! Check karo: {ticket_chan.mention}", ephemeral=True)
    ...
    async def close_ticket_logic(self, interaction: discord.Interaction, reason: str = "Closed via button/command"):
        ...
        if not interaction.response.is_done():
            await interaction.response.send_message("<a:verified_tick:837551087786393710> Ticket close ho gaya hai.", ephemeral=True)
    ...
    async def reopen_ticket_logic(self, interaction: discord.Interaction):
        ...
        if not interaction.response.is_done():
            await interaction.response.send_message("<a:verified_tick:837551087786393710> Ticket reopen ho gaya hai.", ephemeral=True)
    ...
    async def claim_ticket_logic(self, interaction: discord.Interaction):
        ...
        if not interaction.response.is_done():
            await interaction.response.send_message("<a:verified_tick:837551087786393710> Aapne ye ticket claim kar liya hai.", ephemeral=True)
    ...
    async def setup(self, ctx, category: discord.CategoryChannel, support_role: discord.Role, log_channel: discord.TextChannel = None):
        ...
        embed = discord.Embed(
            title="<a:verified_tick:837551087786393710> Ticket System Setup Done!",
    ...
    async def create(self, ctx):
        ...
        await ctx.send(f"<a:verified_tick:837551087786393710> Ticket successfully create ho gaya! Check karo: {ticket_chan.mention}")
    ...
    async def add_member(self, ctx, member: discord.Member):
        ...
        try:
            ...
            await ctx.send(f"<a:verified_tick:837551087786393710> **{member.display_name}** ({member.mention}) ko is ticket me add kar diya gaya hai!")
    ...
    async def logs(self, ctx, limit: int = 10):
        ...
        if not rows:
            return await ctx.send("<a:verified_tick:837551087786393710> Is server me abhi tak koi ticket log record nahi hai.")
