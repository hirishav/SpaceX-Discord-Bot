...
class OwnerMaintenance(commands.Cog):
        ...
        if duration_str == "0" or (duration_str and duration_str.lower() == "off"):
            if server_query:
                ...
                if hasattr(self.bot, 'server_maintenance') and target_guild.id in self.bot.server_maintenance:
                    ...
                    await ctx.send(f"<a:verified_tick:837551087786393710> **Maintenance Manual Override:** `{target_guild.name}` server ko online mode par shift kiya jaa raha hai...")
                else:
                ...
                await ctx.send("<a:verified_tick:837551087786393710> **Maintenance Manual Override:** Bot ko global online mode par shift kiya jaa raha hai...")
    ...
    async def restore_backup(self, ctx):
        ...
        try:
            ...
            await ctx.send("<a:verified_tick:837551087786393710> Backup successfully restored and database reconnected!")
