...
class Welcome(commands.Cog):
    ...
    async def setchannel(self, ctx, channel: discord.TextChannel):
        ...
        embed = discord.Embed(
            title="<a:verified_tick:837551087786393710> Welcome Channel Set!",
    ...
    async def setmessage(self, ctx, *, message: str):
        ...
        embed = discord.Embed(
            title="<a:verified_tick:837551087786393710> Custom Welcome Message Updated!",
    ...
    async def mention(self, ctx, option: str):
        ...
        if opt in ["on", "true", "yes", "enable", "1"]:
            ...
            await ctx.send("<a:verified_tick:837551087786393710> Welcome mention ab **ON** ho gaya hai! (@User tag hoga)")
        elif opt in ["off", "false", "no", "disable", "0"]:
            self.update_config(ctx.guild.id, mention=False)
            await ctx.send("<a:verified_tick:837551087786393710> Welcome mention ab **OFF** ho gaya hai! (Sirf display name dikhega, tag nahi hoga)")
        else:
    ...
    async def enable(self, ctx):
        ...
        await ctx.send("<a:verified_tick:837551087786393710> Welcome system is server me **ENABLE** kar diya gaya hai!")
