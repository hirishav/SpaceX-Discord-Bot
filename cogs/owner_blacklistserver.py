...
class OwnerBlacklist(commands.Cog):
    ...
    async def blacklistserver(self, ctx, server_id: str):
        ...
        try:
            ...
            if guild:
                await guild.leave()
                await ctx.send(f"<a:owner:1453608135104270498> Blacklisted server `{guild.name}` ({server_id}) and left it immediately!")
            else:
                await ctx.send(f"<a:owner:1453608135104270498> Blacklisted server ID `{server_id}` for future invites!")
    ...
    async def whitelistserver(self, ctx, server_id: str):
        ...
        self.bot.db.commit()
        await ctx.send(f"<a:owner:1453608135104270498> Removed server ID `{server_id}` from blacklist!")
