...
class EcoGive(commands.Cog):
    ...
    async def give(self, ctx, member: discord.Member = None, amount_str: str = None):
        ...
        if amount > author_wallet:
            return await ctx.send(f"❌ Aapke paas itne Specie nahi hain! Current wallet: 💠 `{author_wallet}`")
        ...
        await ctx.send(f"💸 {ctx.author.mention} ne {member.mention} ko 💠 `{amount:,}` Specie transfer kar diye!")
