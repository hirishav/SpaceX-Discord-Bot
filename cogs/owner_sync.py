...
class OwnerSync(commands.Cog):
    ...
    async def sync(self, ctx: commands.Context, guilds: commands.Greedy[discord.Object], spec: Optional[Literal["~", "*", "^", "all"]] = None) -> None:
        ...
        if not guilds:
            if spec == "~":
                ...
                if synced_count == 0 and 'first_error' in locals():
                    ...
                    await msg.edit(content=f"<a:giveaway:686211362548088858> **Success!** Forcefully synced slash commands to **{synced_count}/{len(self.bot.guilds)}** servers! Everyone should see them instantly now.")
                return
            else:
                synced = await ctx.bot.tree.sync()

            await ctx.send(f"<a:giveaway:686211362548088858> Synced {len(synced)} commands {'globally' if spec is None else 'to this server'}.")
            return
        ...
        await ctx.send(f"<a:giveaway:686211362548088858> Synced the tree to {ret}/{len(guilds)} servers.")
